...
var Playlists = require('../Models/playlists_model');
var conn = mongoose.connection;
var collectionOfUsers = conn.collection('users');
            ...
            if (result) {
                ...
                query.findOne(function (err, result) {
                    ...
                    if (result) {
                        ...
                        collectionOfUsers.findOneAndUpdate(
                            { email: user.email }, 
        ...
        query.findOne(function (err, result) {
            ...
            if (result) {
                if (result.verificationemail == true) {                            
                    ...
                    collectionOfUsers.findOneAndUpdate({ tokenuser: result.tokenuser, email:result.email}, 
...
app.route('/update')
    .put((req, res) => {
        ...
        collectionOfUsers.findOneAndUpdate(query, { $set: {
            nom: nom,
            prenom: prenom,
            pseudo: pseudo,
            password: hash,
...
app.route('/delete')
    .delete((req, res) => {
        try {  
            let email = req.query["email"];
            var query = { email: email };
            collectionOfUsers.findOneAndDelete(query, function(err) {                
    ...
    .delete((req, res) => {
        try {
            ...
            Playlists.remove(query_id_playlist, function(err) {
                if (err) 
                    return res.status(500).json({ Error: "Playlist " + id_playlist + " is not deleted"});                
